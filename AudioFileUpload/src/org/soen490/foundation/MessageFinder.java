...
public class MessageFinder {
	private final static String SELECT_ALL = 
		...
		private static final String SELECT_BY_RADIUS =
				"SELECT m.mid, m.uid, m.speed, m.latitude, m.longitude, m.created_at, m.user_rating, m.version " +
		...
		private final static String SELECT_AUDIO_FILE =
			"SELECT a.mid, a.path FROM " + MessageTDG.FILE_TABLE + " AS a WHERE a.messageID = ?;";
		
		/**
 		 * Finds a the file path of the message with the passed messageID 
		 * @param messageID Message identifier
		 * @return Returns the ResultSet containing the message id and file path
		 * @throws SQLException
		 */
		public static ResultSet findAudioFile(long mid) throws SQLException {
			PreparedStatement ps = Database.getInstance().getStatement(SELECT_AUDIO_FILE);
			ps.setLong(1, mid);
			ResultSet rs = ps.executeQuery();
			return rs;
		}
