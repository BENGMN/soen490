...
public class AllOrderByDateRetrievalStrategy extends RetrievalStrategy {
	...
	public List<BigInteger> retrieve(double longitude, double latitude,	double speed, int limit) throws NumberFormatException, SQLException {
		...
		int radius = 0;
		messageids = MessageInputMapper.findIdsInProximityOrderDate(longitude, latitude, radius, limit);
		
		if (messageids.size() < getMinMessages()) 
			messageids = MessageInputMapper.findIdsInProximityOrderDate(longitude, latitude, getIncreasedRadius(radius, speed), limit);
